...
// buildStories.coffee
var buildOneStory, checkSentence, dictPath, dir, hPrinted, inclChinese, inclWords, lLines, logChinese, logEnglish, main, maxENlen, maxZHlen, printSentence, storiesDir, translator;
...
// ---------------------------------------------------------------------------
buildOneStory = function(filename, dir) {
  var contents, hPara, hSent, hStory, i, inPath, j, k, l, len1, len2, mapped, nFound, outPath, ref, ref1;
...
// ---------------------------------------------------------------------------
printSentence = function(hSent, nPar, nSent) {
  var _, k, lFound, len1, trans, word;
  // --- Returns total number of words/phrases looked up
  ...
  if (inclWords && (lFound.length > 0)) {
    log('');
    for (k = 0, len1 = lFound.length; k < len1; k++) {
      [word, trans, _, _] = lFound[k];
      if (hPrinted[word] == null) {
        log(`   (${word} ${trans})`);
        hPrinted[word] = true;
      }
    }
...
// ---------------------------------------------------------------------------
checkSentence = function(hSent, nPar, nSent) {
  if (hSent.en === undef) {
...
// ---------------------------------------------------------------------------
logEnglish = function(line) {
  var k, lWords, len, len1, lineNum, ref, word;
